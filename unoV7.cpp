...
{
    cout << "It's your turn!";
    cout << setw(17) << " " << "Active Card: ";
    crdCnv(actvArr, ROW);
    cout << endl
         << endl;
    bubSrt(plyrHnd, ROW, actvDsp, srtdHnd);
    cout << endl
         << "| Cards in your hand: " << plyrCnt << "  | Opponent number of cards: " << npcCnt << " |" << endl
         << endl;

    // Prompt for how player wants to proceed
    cout << setw(16) << " " << "What would you like to do?" << endl;
    cout << "|  R: Play Red    |   B: Play Blue   |   Y: Play Yellow  |" << endl;
    cout << "|  G: Play Green  |   W: Play Wild   |    D: Draw Card   | " << endl;
...
void plyrTrn(int plyrHnd[][COL], int npcHnd[][COL], int srtdHnd[][COL], int actvArr[][COL], string &actvDsp, char colSel, int numSel, int &plyrCnt, int &npcCnt, int ROW, bool &turn)
{
    ...
    while (!vldPly) // Loop until a valid play is made
    {
        ...
        cout << setw(19) << " " << "Choose a card color" << endl;
        cin >> colSel;
        if (cin.fail())
        {
            cin.clear();

...
void readScrs()
{
    ...
    // v5.2: for_each replaces the manual map iterator walk. Second
    // non-mutating algorithm demo (count_if was v5.1 Step 02). The lambda
    // captures the running record number by reference; map's bidirectional
    // iterators satisfy for_each's InputIterator requirement, and records
    // still print in name order because map keeps its keys sorted.
    int count = 1;
    for_each(scores.begin(), scores.end(),
             [&count](const pair<const string, Scores> &rec)
             {
